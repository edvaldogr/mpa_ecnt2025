...
path_hedge <- here::here("dados/limpos/dados_hedge_clean.rds")
dados_hedge_clean <- readr::read_rds(path_hedge)

## importa dados_capm_clean no formato .rds para análise
path_clean <- here::here("dados/limpos/dados_capm_clean.rds")
...
séries de retornos são similares, conforme os retornos médios, 
...
hedge de [`r round(confint(lm_hedge)["ret_futuro", 1], 2)`, `r round(confint(lm_hedge)["ret_futuro", 2], 2)`], 
implica que o investidor deve proteger entre 96% e 99% de sua posição longa no S&P500 
com uma posição curta em contratos futuros. O intervalo estreito sugere alta confiança na estimativa 
...
Com uma estatística F igual 2,39 e um valor-p correspondente de 0,1237, 
...
aproximadamente uma distribuição normal. Entretanto, como a série 
de retornos excedentes do S&P (500) apresenta assimetria negativa e exceso de 
curtose, vamos obter uma estimativa pontual do coeficiente de correlação de 
Spearman (que não supoe uma distribuição normal para os dados) para 
...
`r round(r_spearman, 2)`indicando ainda uma correlação linear positiva considerável, 
mas um pouco menor, entre as duas séries.

O gráfico de dispersão entre os retornos excedentes da Ford e do S&P (500) 
indica correlação linear positiva , entre as duas séries ($r = 0.58$), 
...
pode não apresentar problemas de ajuste para modelar a relação entre os retornos 
...
conforme previsto pelo CAPM.


## Interpretação do Beta no CAPM

O **beta ($\beta$)**, no contexto do CAPM, mede a sensibilidade do retorno das ações 
da Ford em relação ao retorno do portfólio de mercado. Em outras palavras, o beta 
indica o grau de exposição ao risco de mercado (ou **risco sistemático**) — aquele 
que não pode ser eliminado pela diversificação.

É uma medida de quanto o ativo tende a se mover em resposta às variações do mercado.

- **$\beta = 1$:** As ações da Ford têm um comportamento **em linha com o mercado**. 
Se o mercado sobe (ou cai) 1%, espera-se que as ações da Ford se valorizem (ou desvalorizem) 
também em cerca de 1%. Isso indica que a Ford possui exposição média ao risco de mercado.

- **$\beta > 1$:** As ações da Ford apresentam maior volatilidade e maior sensibilidade ao mercado.
Quando o mercado sobe 1%, o papel tende a subir mais de 1%; quando o mercado cai 1%, tende a cair
mais de 1%. Esse comportamento caracteriza um ativo agressivo, com beta elevado, que amplifica os 
movimentos do mercado — ou seja, assume maior risco, com potencial de ganhos superiores em ciclos de 
alta, mas também maiores perdas em períodos de queda

- **$\beta < 1$:** As ações da Ford são menos sensíveis às oscilações do mercado, apresentando 
menor volatilidade relativa**.  O papel tende a oscilar menos do que o índice de referência. 
Isso caracteriza um ativo defensivo, com menor exposição ao risco de mercado.

- **$\beta < 0$:** Embora incomum, um beta negativo indica que o papel tende a se mover na direção 
oposta ao mercado**. Quando o mercado sobe, o ativo tende a cair, e vice-versa. 
Isso sugere um comportamento **anticíclico**, podendo atuar como instrumento de hedge em uma 
carteira diversificada.

Conforme o resultado anterior, o intervalo de confiança de 95% para o beta estimado da Ford foi  
[`r round(confint(capm_ford)["retexc_sp500", 1], 2)` , `r round(confint(capm_ford)["retexc_sp500", 2], 2)`].  
Isso significa que, com 95% de confiança, o beta da Ford é superior a 1, o que indica que suas ações possuem 
perfil mais agressivo, com volatilidade superior à do mercado e maior exposição ao risco sistemático.  
Consequentemente, a Ford tende a superar o mercado em ciclos de alta, mas também a r
egistrar quedas mais acentuadas em períodos de baixa.
...
Extraia os resíduos e os valores ajustados do modelo `capm_ford`, faça um 
gráfico de dispersão entre eles e analise se há alguma evidência de 
viés ou de heterocedasticidade neste modelo.



# Exercício 2 {.unnumbered}

Faça um gráfico quantil-quantil normal dos resíduos do modelo `capm_ford` e 
analise o resultado.
...
Implemente o teste de Jarque-Bera sobre os resíduos do modelo `capm_ford` e 
analise o resultado.
